...
unsigned long soczewka_global_padding = 0;
EXPORT_SYMBOL(soczewka_global_padding);
...
void __init soczewka_init_globals(void)
{
	...
	for (cur = soczewka; *cur != '\0';) {
		...
		if (soczewka_global_words_count == CONFIG_SOCZEWKA_MAX_WORDS) {
			pr_warn("Too many Soczewka (tm) bad words. "
					"Ignoring rest of the words.");
			break;
		}
		...
		soczewka_global_padding = max(soczewka_global_padding, len - 1);
	}

	init_task.soczewka.words_count = soczewka_global_words_count;
	init_task.soczewka.words = soczewka_global_words;
	init_task.soczewka.padding = soczewka_global_padding;
}
